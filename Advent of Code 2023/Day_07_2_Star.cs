namespace Advent_of_Code_2023
{
    public class Day_07_2_Star
    {
        ...
        private static string ConvertJokers(string hand)
        {
            ...
            if (cards['J'] == 1)
            {
                if (cards.Count == 3)
                {
                    foreach (KeyValuePair<char, int> ch in cards)
                    {
                        if (ch.Value > highestValue)
                        {
                            highestValue = ch.Value;
                        }
                    }
